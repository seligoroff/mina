...
Mina предоставляет единую CLI утилиту `cli.py` с командами:
...
4. **`tg-login`** — Инициализация Telethon-сессии (однократно, интерактивно)
5. **`tg-list`** — Список доступных диалогов/каналов из Telegram-сессии
6. **`export-chat`** — Экспорт переписки Telegram в JSONL/CSV
...
- pip-библиотеки:
  ...
  - `pymorphy3` и `pymorphy3-dicts-ru` - для лемматизации в tag
...
```bash
python cli.py scribe -i <аудиофайл> -o <текстовый_вывод> [-m <модель>]
```
...
> 💡 Теперь обработка вынесена в отдельные уровни: CLI только читает файлы и выводит результат, а вся бизнес-логика (лемматизация, фильтры) живёт в `WordAnalysisService`. Это упрощает повторное использование (например, из API) и тестирование.

---

### 3. Создание протокола (`protocol`)

Создает структурированный протокол из расшифровки с помощью LLM провайдера (по умолчанию — DeepSeek). Архитектура позволяет подключать и другие источники (например, OpenAI или локальные модели) через конфиг.

```bash
python cli.py protocol -i <файл_расшифровки> [-o <выходной_файл>] [--config <путь_к_конфигу>] [--instructions <файл>] [--extra "<текст>"]
```
...
# С альтернативными инструкциями (например, для JSONL-экспорта Telegram)
python cli.py protocol -i chat.jsonl -o protocol.md -I resources/protocol/telegram-chat-instructions.md

# Добавить важное уточнение к инструкциям на лету
python cli.py protocol -i chat.jsonl -o protocol.md -I resources/protocol/telegram-chat-instructions.md -E "Сфокусируйся на проблемах с видео"
```
...
| `--instructions, -I` | Путь к альтернативному файлу инструкций (переопределяет конфиг) |
| `--extra, -E` | Текст-уточнение, который будет добавлен в конец инструкций |
...
# Пример альтернативного провайдера (заготовка)
# openai:
#   api_key: "..."
#   model: "gpt-4o"
#   instructions: "openai-protocol-instructions.md"
#   temperature: 0.5
#   max_tokens: 2048
```
...
4. **Экспорт чата Telegram и генерация протокола:**
   ```bash
   # 1) Инициализация сессии (первый раз)
   python cli.py tg-login
   # 2) Список доступных диалогов (для подбора peer)
   python cli.py tg-list --limit 50
   # 3) Экспорт сообщений за период
   python cli.py export-chat -p @username_or_id -o chat.jsonl --from 2025-11-14 --to 2025-11-16 --format jsonl
   # 4) Генерация протокола с профилем для Telegram
   python cli.py protocol -i chat.jsonl -o protocol.md -I resources/protocol/telegram-chat-instructions.md
   ```
...
```
Файл `.coverage` будет размещён в `tests/.coverage`.
...
│   ├── adapters/                    # Адаптеры для внешних библиотек
│   ├── application/                 # Бизнес-логика (сервисы, порты)
...
├── resources/
│   └── protocol/
│       ├── deepseek-protocol-instructions.md           # Инструкции для обычных транскриптов
│       └── telegram-chat-instructions.md               # Инструкции для JSONL-выгрузки Telegram
